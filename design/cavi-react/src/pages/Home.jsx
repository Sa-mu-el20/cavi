...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg }}>
      ...
      {/* HERO */}
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '84px 40px 56px', display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', gap: 64, alignItems: 'center' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#e6eed6', color: colors.greenText, padding: '7px 14px', borderRadius: 999, fontSize: 13, fontWeight: 600, marginBottom: 26 }}>
          ...
          <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
            <button onClick={() => navigate('/login')} style={{ background: colors.orange, color: '#fff', border: 'none', borderRadius: 12, padding: '16px 30px', fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', gap: 30, marginTop: 44 }}>
            {[['95', 'corretores ativos'], ['2.400+', 'imóveis publicados'], ['12', 'cidades']].map(([v, l], i) => (
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 18, boxShadow: '0 30px 60px -28px rgba(60,45,25,0.30)', overflow: 'hidden' }}>
            <div style={{ height: 40, background: colors.cream, borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', gap: 7, padding: '0 16px' }}>
              ...
              <span style={{ marginLeft: 12, fontSize: 12, color: colors.faint }}>cavi.com.br/albuquerque</span>
            </div>
            <img src="/assets/prop-02.png" alt="" style={{ width: '100%', height: 200, objectFit: 'cover', display: 'block' }} />
            <div style={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: colors.orange, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15 }}>A</div>
                <div style={{ fontFamily: fonts.display, fontWeight: 500, fontSize: 17 }}>Albuquerque Imóveis</div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[['/assets/prop-01.png', 'R$ 890.000', '2 quartos · 78m²'], ['/assets/prop-04.png', 'R$ 1.180.000', '3 quartos · 110m²']].map(([src, p, d]) => (
                  <div key={src} style={{ border: `1px solid ${colors.borderSoft}`, borderRadius: 10, overflow: 'hidden' }}>
                    <img src={src} style={{ width: '100%', height: 64, objectFit: 'cover', display: 'block' }} />
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '56px 40px 40px' }}>
        <div style={{ textAlign: 'center', marginBottom: 44 }}>
          ...
          <h2 style={{ fontFamily: fonts.display, fontWeight: 300, fontSize: 38, margin: 0, letterSpacing: -0.5 }}>No ar em três passos</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 28 }}>
          {PASSOS.map((p) => (
            <div key={p.n} style={{ background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 16, padding: 34 }}>
              <div style={{ fontFamily: fonts.display, fontSize: 15, fontWeight: 600, color: colors.orange, marginBottom: 18 }}>{p.n}</div>
              <h3 style={{ fontFamily: fonts.display, fontWeight: 500, fontSize: 22, margin: '0 0 10px' }}>{p.t}</h3>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '56px 40px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
          {corretores.map((c) => <BrokerCard key={c.id} corretor={c} />)}
        </div>
      </section>

      {/* CTA */}
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 40px 80px' }}>
        <div style={{ background: colors.ink, borderRadius: 22, padding: 64, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <footer style={{ borderTop: `1px solid ${colors.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <img src="/assets/logo-ink.png" alt="CAVI" style={{ height: 24, opacity: 0.85 }} />
          <div style={{ fontSize: 13, color: colors.mutedSoft }}>
            © 2026 CAVI · Termos de uso · Privacidade · <span style={{ cursor: 'pointer' }} onClick={() => navigate('/admin')}>Admin</span>
          </div>
        </div>
      </footer>
    </div>
