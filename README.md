...
> SaaS imobiliário: cada corretor tem seu próprio catálogo de imóveis online.
...
3. Cole o endereço do repositório:
   ...
   ```
   https://github.com/Sa-mu-el20/cavi.git
   ```
...
> Alternativa pelo terminal: `git clone https://github.com/Sa-mu-el20/cavi.git` e depois **Arquivo → Abrir Pasta** no VSCode.
...
O backend sobe em **http://localhost:8411**. A documentação da API (Swagger) fica em **http://localhost:8411/docs**.
...
O frontend abre em **http://localhost:5181**. **É por esse endereço que você acessa o sistema no navegador.** (Ele conversa sozinho com o backend.)
...
- **Aluno 1 → CRUD N:N de Comodidades do imóvel**
  Tutorial: [`docs/TutorialAluno1.md`](docs/TutorialAluno1.md)
- **Aluno 2 → Contador de visualizações e ordenação do catálogo**
