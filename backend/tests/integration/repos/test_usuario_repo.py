...
class TestUsuarioRepoInserir:
    ...
    def test_inserir_usuario_com_perfil_corretor(self):
        """Deve inserir usuário corretor corretamente."""
        usuario = Usuario(
            id=0,
            nome="Corretor Teste",
        ...
        assert usuario_salvo.perfil == Perfil.CORRETOR.value
...
class TestUsuarioRepoAlterar:
    ...
    def test_alterar_perfil_usuario(self):
        ...
        usuario.id = usuario_id
        usuario.perfil = Perfil.ADMIN.value
        resultado = usuario_repo.alterar(usuario)

        assert resultado is True
        usuario_alterado = usuario_repo.obter_por_id(usuario_id)
        assert usuario_alterado.perfil == Perfil.ADMIN.value
...
class TestUsuarioRepoObterTodosPorPerfil:
    ...
    def test_obter_todos_por_perfil_corretor(self):
        """Deve retornar apenas usuários com perfil Corretor."""
        # Inserir usuários de diferentes perfis
        corretor = Usuario(
            id=0,
            nome="Corretor Perfil",
            email="corretor_perfil@example.com",
            senha=criar_hash_senha("Senha@123"),
            perfil=Perfil.CORRETOR.value
        )
        admin = Usuario(
            id=0,
            nome="Admin Perfil",
            email="admin_perfil@example.com",
            senha=criar_hash_senha("Senha@123"),
            perfil=Perfil.ADMIN.value
        )
        usuario_repo.inserir(corretor)
        usuario_repo.inserir(admin)

        resultado = usuario_repo.obter_todos_por_perfil(Perfil.CORRETOR.value)

        assert isinstance(resultado, list)
        for usuario in resultado:
            assert usuario.perfil == Perfil.CORRETOR.value

    def test_obter_todos_por_perfil_admin(self):
        """Deve retornar apenas usuários com perfil Administrador."""
        admin = Usuario(
            id=0,
            nome="Admin Filtro",
            email="admin_filtro@example.com",
            senha=criar_hash_senha("Senha@123"),
            perfil=Perfil.ADMIN.value
        )
        usuario_repo.inserir(admin)

        resultado = usuario_repo.obter_todos_por_perfil(Perfil.ADMIN.value)
