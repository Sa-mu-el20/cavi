...
class TestSQLInjection:
    ...
    def test_obter_recurso_id_nao_numerico_retorna_422(self, client):
        """ID não numérico no path deve ser rejeitado pela validação (422)"""
        response = client.get("/api/publico/imoveis/1 OR 1=1")
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
class TestEscalacaoPrivilegios:
    ...
    def test_nao_autenticado_em_area_admin_retorna_401(self, client):
        ...
        response = client.get("/api/admin/corretores")
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_cliente_nao_acessa_admin_corretores_retorna_403(self, cliente_autenticado):
        """Corretor autenticado em área admin deve receber 403"""
        response = cliente_autenticado.get("/api/admin/corretores")
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_cliente_nao_executa_mutacao_admin_retorna_403(self, cliente_autenticado):
        """Não-admin não deve poder mutar recursos via endpoint admin (403)"""
        token = _csrf(cliente_autenticado)
        response = cliente_autenticado.patch(
            "/api/admin/corretores/1/status",
            json={"status": "Inativo"},
            headers={"X-CSRF-Token": token},
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_vendedor_nao_acessa_area_admin_retorna_403(self, vendedor_autenticado):
        """Corretor secundário não deve acessar área admin (admin-only)"""
        response = vendedor_autenticado.get("/api/admin/corretores")
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_admin_acessa_admin_corretores_retorna_200(self, admin_autenticado):
        """Admin deve acessar a listagem de corretores (200)"""
        response = admin_autenticado.get("/api/admin/corretores")
